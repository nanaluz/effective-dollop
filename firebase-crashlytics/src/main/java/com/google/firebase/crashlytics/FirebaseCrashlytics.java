...
/**
 * The Firebase Crashlytics API provides methods to annotate and manage fatal
 * and non-fatal reports captured and reported to Firebase Crashlytics.
 *
 * <p>By default, Firebase Crashlytics is automatically initialized.
 *
 * <p>Call {@link FirebaseCrashlytics#getInstance()} to get the singleton
...
public class FirebaseCrashlytics {
  ...
  static @Nullable FirebaseCrashlytics
  init(@NonNull FirebaseApp app,
    ...
    final IdManager idManager =
        new IdManager(context, appIdentifier, firebaseInstallationsApi);
    ...
    final Onboarding onboarding =
        new Onboarding(app, context, idManager, arbiter);
    ...
    if (analyticsConnector != null) {
      // If FA is available, create a logger to log events from the Crashlytics
      // origin.
      ...
      // Create a listener to register for events coming from FA, which supplies
      // both breadcrumbs as well as Crashlytics-origin events through different
      // streams.
      final CrashlyticsAnalyticsListener crashlyticsAnalyticsListener =
          new CrashlyticsAnalyticsListener();

      // Registering our listener with FA should return a "handle", in which
      // case we know we've registered successfully. Subsequent calls to
      // register a listener will return null.
      final AnalyticsConnectorHandle analyticsConnectorHandle =
          subscribeToAnalyticsEvents(analyticsConnector,
                                     crashlyticsAnalyticsListener);

      if (analyticsConnectorHandle != null) {
        Logger.getLogger().d(
            "Firebase Analytics listener registered successfully.");
        // Create the event receiver which will supply breadcrumb events to
        // Crashlytics
        final BreadcrumbAnalyticsEventReceiver breadcrumbReceiver =
            new BreadcrumbAnalyticsEventReceiver();
        // Logging events to FA is an asynchronous operation. This logger will
        // send events to FA and block until FA returns the same event back to
        // us, from the Crashlytics origin. However, in the case that data
        // collection has been disabled on FA, we will not receive the event
        // back (it will be silently dropped), so we set up a short timeout
        // after which we will assume that FA data collection is disabled and
        // move on.
        final BlockingAnalyticsEventLogger blockingAnalyticsEventLogger =
            new BlockingAnalyticsEventLogger(directAnalyticsEventLogger,
                                             APP_EXCEPTION_CALLBACK_TIMEOUT_MS,
                                             TimeUnit.MILLISECONDS);

        // Set the appropriate event receivers to receive events from the FA
        // listener
        crashlyticsAnalyticsListener.setBreadcrumbEventReceiver(
            breadcrumbReceiver);
        crashlyticsAnalyticsListener.setCrashlyticsOriginEventReceiver(
            blockingAnalyticsEventLogger);

        // Set the breadcrumb event receiver as the breadcrumb source for
        // Crashlytics.
        ...
        Logger.getLogger().d(
            "Firebase Analytics listener registration failed.");
        ...
        // We cannot listen for Crashlytics origin events, but we can still send
        // events, so set the non-blocking analytics event logger for
        // Crashlytics.
        analyticsEventLogger = directAnalyticsEventLogger;
      }
    } else {
      // FA is entirely unavailable. We cannot listen for breadcrumbs or send
      // events.
    ...
    final CrashlyticsCore core = new CrashlyticsCore(
        app, idManager, nativeComponent, arbiter, breadcrumbSource,
        analyticsEventLogger, crashHandlerExecutor);
    ...
    final boolean finishCoreInBackground =
        core.onPreExecute(settingsController);
  ...
  private static AnalyticsConnectorHandle
  subscribeToAnalyticsEvents(@NonNull AnalyticsConnector analyticsConnector,
    ...
    if (handle == null) {
      ...
      // Older versions of FA don't support CRASHLYTICS_ORIGIN. We can try using
      // the old Firebase Crash Reporting origin
      handle = analyticsConnector.registerAnalyticsConnectorListener(
          LEGACY_CRASH_ANALYTICS_ORIGIN, listener);

      // If FA allows us to connect with the legacy origin, but not the new one,
      // nudge customers to update their FA version.
      if (handle != null) {
        Logger.getLogger().w(
   ...
   *
   * <p>The default {@link FirebaseApp} instance must be initialized before this
   * function is called. See <a
  ...
  public void recordException(@NonNull Throwable throwable) {
    if (throwable ==
        null) { // Users could call this with null despite the annotation.
      Logger.getLogger().w(
          "Crashlytics is ignoring a request to log a null exception.");
      return;
    }
   ...
   *
   * <p>Logs are visible in the session view on the Firebase Crashlytics
   * console.
   *
   * <p>Newline characters are stripped and extremely long messages are
   * truncated. The maximum log size is 64k. If exceeded, the log rolls such
   * that messages are removed, starting from the oldest.
   *
   * @param message the message to be logged
   */
  public void log(@NonNull String message) { core.log(message); }

  /**
   * Records a user ID (identifier) that's associated with subsequent fatal and
   * non-fatal reports.
   *
   * <p>The user ID is visible in the session view on the Firebase Crashlytics
   * console.
   *
   ...
   * non-fatal reports.
   *
   * <p>Multiple calls to this method with the same key update the value for
   * that key.
   *
   * <p>The value of any key at the time of a fatal or non-fatal event is
   * associated with that event.
   *
   * <p>Keys and associated values are visible in the session view on the
   * Firebase Crashlytics console.
   *
   * <p>Accepts a maximum of 64 key/value pairs. New keys beyond that limit are
   ...
   * non-fatal reports.
   *
   * <p>Multiple calls to this method with the same key will update the value
   * for that key.
   *
   * <p>The value of any key at the time of a fatal or non-fatal event will be
   * associated with that event.
   *
   * <p>Keys and associated values are visible in the session view on the
   * Firebase Crashlytics console.
   *
   * <p>A maximum of 64 key/value pairs can be written, and new keys added
   ...
   * been sent to Crashlytics. If automatic data collection is enabled, then
   * reports are uploaded automatically and this always returns false. If
   * automatic data collection is disabled, this method can be used to check
   * whether the user opts-in to send crash reports from their device.
   *
   ...
   * reports on a device to send to Crashlytics. Otherwise, this method is a
   * no-op.
   */
  public void sendUnsentReports() { core.sendUnsentReports(); }

  /**
   * If automatic data collection is disabled, this method queues up all the
   * reports on a device for deletion. Otherwise, this method is a no-op.
   */
  public void deleteUnsentReports() { core.deleteUnsentReports(); }
   ...
   *
   * <p>If this is set, it overrides the data collection settings provided by
   * the Android Manifest, as well as any Firebase-wide automatic data
   * collection settings.
   *
   * <p>If automatic data collection is disabled for Crashlytics, crash reports
   * are stored on the device. To check for reports, use the {@link
   * #checkForUnsentReports()} method. Use {@link #sendUnsentReports()} to
   * upload existing reports even when automatic data collection is disabled.
   * Use {@link #deleteUnsentReports()} to delete any reports stored on the
   * device without sending them to Crashlytics.
   *
   * @param enabled whether to enable automatic data collection. When set to
   *     `false`, the new value does not apply until the next run of the app. If
   *     you want to disable data collection by default for all app runs, add
   *     the `firebase_crashlytics_collection_enabled` flag to your app's
   *     AndroidManifest.xml.
   */
