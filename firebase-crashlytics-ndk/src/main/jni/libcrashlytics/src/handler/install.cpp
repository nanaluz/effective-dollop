...
inline void make_suppliment_path_from(const char *path, const char *suffix,
                                      char (&buffer)[N]) {
...
void finalize() {
  ...
  detail::breakpad_context *context =
      reinterpret_cast<detail::breakpad_context *>(detail::instance.load());
  delete context;
}
...
detail::breakpad_context::breakpad_context(
    ...
    : handler_context_(handler_context), descriptor_(handler_context.filename),
      handler_(descriptor_, NULL, callback,
               reinterpret_cast<void *>(&handler_context_), true, -1) {}
...
inline void write_supplimentary_file(const detail::context &handler_context,
  ...
  DEBUG_OUT("Supplementary file with suffix '%s' is at: %s", suffix,
            supplimentary_path);

  int fd;
  if ((fd = google::crashlytics::detail::open(supplimentary_path)) == -1) {
    DEBUG_OUT("Couldn't open supplementary file '%s'; %s", supplimentary_path,
              std::strerror(errno));
    return;
  }
...
bool detail::breakpad_context::callback(
    const google_breakpad::MinidumpDescriptor &descriptor, void *context,
    bool succeeded) {
  const detail::context &handler_context =
      *reinterpret_cast<detail::context *>(context);

  DEBUG_OUT("Path is: %s %s", descriptor.path(),
            succeeded ? "succeeded" : "failed");

  detail::write_supplimentary_file(
      handler_context, descriptor.path(), ".device_info",
      [](const detail::context &handler_context, int fd) {
        google::crashlytics::handler::write_device_info(handler_context, fd);
      });

  detail::write_supplimentary_file(
      handler_context, descriptor.path(), ".maps",
      [](const detail::context &handler_context, int fd) {
...
bool install_signal_handler(const detail::context &handler_context) {
  if (detail::instance.load() == nullptr) {
    detail::instance.store(reinterpret_cast<void *>(
        crashlytics::detail::memory::allocate_storage<detail::breakpad_context>(
            handler_context)));
  }
...
//! When building in debug, we can search for this symbol in the output of
//! readelf or objdump, to verify
