...
//! Since the handler is serial, we can reuse already allocated static duration
//! storage.
template <typename T> inline void *make_function_scoped_static_byte_array() {
  DEBUG_OUT(
      "Couldn't use the page allocator, returning static storage of size %u",
...
template <typename T> inline T *get_static_storage() {
  return new (make_function_scoped_static_byte_array<T>()) T();
}

template <typename T, typename U> inline T *get_static_storage(U &&initial) {
  return new (make_function_scoped_static_byte_array<T>())
      T(std::move(initial));
}

//! Allocate storage from the kernel if possible, otherwise make a best effort
//! attempt by
//  providing static storage. Keep in mind that this allocates new memory _once_
//  for every new type. Calling this for the same type more than once will
//  return the same storage.
template <typename T> inline T *allocate_storage() noexcept {
...
inline T *allocate_storage(U &&initial) noexcept {
  ...
  T *value = storage != nullptr
                 ? storage
                 : get_static_storage<T>(std::forward<U>(initial));
  return value;
}

template <typename T> inline void release_storage(T *storage) {
